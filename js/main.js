...
function loop() {
  try {
    ...
    const me = state.me;
    if (me) {
      // 1) Joystick (mobile) prioritaire lorsqu'un touch est actif
      if (input && input.activeTouchId !== null && Number.isFinite(input.targetDir)) {
      ...
      // 3) Sinon, clavier
        ...
        if (Number.isFinite(input.targetDir)) me.targetDir = input.targetDir;
      }
